...
def build_mcp_server(**kw) -> Any:
    ...
    @tool(
        ...
        description=(
            ...
            "\n"
            "DIRECTION RULE — when peer ALREADY OWES THE USER (net > 0): "
            "DO NOT emit 'request' and DO NOT emit 'settle_up' for that peer. "
            "Both create a NEW pending bunq request on top of the existing "
            "ones, which double-charges from the peer's POV. Use "
            "send_chat_message instead to nudge them in their dm — the "
            "peer will then accept the request that's already pending. "
            "Only use 'settle_up' when net < 0 (the user owes the peer): in "
            "that case it fires a real bunq Payment to settle. "
            "\n"
